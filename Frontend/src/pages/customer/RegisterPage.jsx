...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    setIsLoading(true);
    try {
      await api.post('/auth/register', { name, phone, email });
      setStep(2);
    } catch (err) {
      setError(err.response?.data?.message || 'Gagal mengirim OTP. Silakan coba lagi.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleVerify = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await api.post('/auth/verify-otp', { phone, code: otp, name, email });
      ...
      <div className="w-full max-w-md mx-auto space-y-6 md:space-y-8 bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-neutral-100">
        ...
        <div className="space-y-2 text-center">
          ...
          <p className="text-sm md:text-base text-neutral-500">
            {step === 1
              ? 'Daftar untuk mulai memesan layanan laundry kami'
        ...
        {step === 1 ? (
          <form onSubmit={handleRegister} className="space-y-4 md:space-y-5">
            ...
            <p className="text-center text-sm md:text-base text-neutral-500 pt-2 md:pt-3">
              Sudah punya akun?{' '}
              <Link to="/customer/login" className="text-neutral-900 hover:underline font-medium">
                Masuk di sini
              </Link>
            </p>
          </form>
